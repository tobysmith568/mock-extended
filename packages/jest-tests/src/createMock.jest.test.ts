...
describe("mock-extended with Jest", () => {
  ...
  test("creates method mocks lazily from jest.fn() factory", () => {
  ...
  test("supports mockReturnValue on generated jest.fn() methods", () => {
    const factory = jest.fn(() => jest.fn());
    const mock = createMock(factory);
    const dep = mock<Dependency>();
  ...
  test("exposes Jest call metadata via mock.calls", () => {
    const factory = jest.fn(() => jest.fn());
    const mock = createMock(factory);
    const dep = mock<Dependency>();
  ...
  test("supports Jest-specific matcher toHaveBeenLastCalledWith", () => {
    ...
    expect(dep.doWork).toHaveBeenLastCalledWith("def");
  });
